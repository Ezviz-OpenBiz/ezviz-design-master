...
describe('Notification.placement', () => {
  ...
  describe('placement', () => {
    it('can be configured per notification using the `open` method', () => {
      ...
      style = getStyle($$('.ezd-notification-topLeft')[0]);
      ...
      expect($$('.ezd-notification-topLeft').length).toBe(1);

      // topRight
      open({
        placement: 'topRight',
      });
      style = getStyle($$('.ezd-notification-topRight')[0]);
      ...
      expect($$('.ezd-notification-topRight').length).toBe(1);
      ...
      style = getStyle($$('.ezd-notification-bottomRight')[0]);
      ...
      expect($$('.ezd-notification-bottomRight').length).toBe(1);

      // bottomLeft
      open({
        placement: 'bottomLeft',
      });
      style = getStyle($$('.ezd-notification-bottomLeft')[0]);
      expect(style.top).toBe('');
      expect(style.left).toBe('0px');
      ...
      expect($$('.ezd-notification-bottomLeft').length).toBe(1);
    });
  ...
  describe('mountNode', () => {
    ...
    it('can be configured per notification using the `open` method', () => {
      ...
      expect($container.querySelector('.ezd-notification')).not.toBe(null);
      ...
      setTimeout(() => {
        ...
        open();
        expect($container.querySelector('.ezd-notification')).toBe(null);
      });
    });
