...
describe('notification', () => {
  ...
  it('should be able to hide manually', async () => {
    ...
    expect(document.querySelectorAll('.ezd-notification-notice').length).toBe(2);

    notification.close('1');
    jest.runAllTimers();
    expect((await getInstance('ezd-notification-topRight')).component.state.notices).toHaveLength(
      1,
    );
  ...
  it('should be able to destroy globally', async () => {
    ...
    await Promise.resolve();
    expect(document.querySelectorAll('.ezd-notification').length).toBe(1);
    expect(document.querySelectorAll('.ezd-notification-notice').length).toBe(2);
    notification.destroy();
    await Promise.resolve();
    expect(document.querySelectorAll('.ezd-notification').length).toBe(0);
    expect(document.querySelectorAll('.ezd-notification-notice').length).toBe(0);
  });
  ...
  it('should be able to config rtl', () => {
    ...
    expect(document.querySelectorAll('.ezd-notification-rtl').length).toBe(1);
  });
  ...
  it('should be able to global config rootPrefixCls', () => {
    ...
    expect(document.querySelectorAll('.ezd-notification-notice')).toHaveLength(0);
  ...
  it('should be able to open with icon', async () => {
    const openNotificationWithIcon = async type => {
      const iconPrefix = '.ezd-notification-notice-icon';
      notification[type]({
        message: 'Notification Title',
        duration: 0,
  ...
  it('should be able to add parent class for different notification types', async () => {
    const openNotificationWithIcon = async type => {
      ...
      await Promise.resolve();
      expect(document.querySelectorAll(`.ezd-notification-notice-${type}`).length).toBe(1);
    };
  ...
  it('support icon', () => {
    ...
    expect(document.querySelectorAll('.ezdicon-user').length).toBe(1);
  });
});
