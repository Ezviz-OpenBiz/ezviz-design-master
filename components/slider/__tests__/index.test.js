...
describe('Slider', () => {
  ...
  it('should show tooltip when hovering slider handler', () => {
    ...
    wrapper.find('.ezd-slider-handle').at(0).simulate('mouseEnter');
    expect(render(wrapper.find('Trigger').instance().getComponent())).toMatchSnapshot();
    wrapper.find('.ezd-slider-handle').at(0).simulate('mouseLeave');
    expect(render(wrapper.find('Trigger').instance().getComponent())).toMatchSnapshot();
  });
  ...
  it('when tooltipVisible is true, tooltip should show always, or should never show', () => {
    ...
    expect(wrapper.find('.ezd-tooltip-content').at(0).hasClass('ezd-tooltip-hidden')).toBe(false);
    wrapper.find('.ezd-slider-handle').at(0).simulate('mouseEnter');
    expect(wrapper.find('.ezd-tooltip-content').at(0).hasClass('ezd-tooltip-hidden')).toBe(false);
    wrapper.find('.ezd-slider-handle').at(0).simulate('click');
    expect(wrapper.find('.ezd-tooltip-content').at(0).hasClass('ezd-tooltip-hidden')).toBe(false);
    wrapper = mount(<Slider defaultValue={30} tooltipVisible={false} />);
    expect(wrapper.find('.ezd-tooltip-content').length).toBe(0);
  });
  ...
  it('when step is null, thumb can only be slided to the specific mark', () => {
    ...
    expect(wrapper.find('.ezd-slider-handle').get(0).props).toHaveProperty('value', 48);
  });
  ...
  it('when step is not null, thumb can be slided to the multiples of step', () => {
    ...
    expect(wrapper.find('.ezd-slider-handle').get(0).props).toHaveProperty('value', 49);
  });
