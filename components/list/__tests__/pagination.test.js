...
describe('List.pagination', () => {
  ...
  function renderedNames(wrapper) {
    return wrapper.find('.ezd-list-item').map(row => row.text());
  }
  ...
  it('specify the position of pagination', () => {
    ...
    expect(wrapper.find('.ezd-list').childAt(0).find('.ezd-pagination')).toHaveLength(1);
    wrapper.setProps({ pagination: { position: 'bottom' } });
    expect(wrapper.find('.ezd-list').children().last().find('.ezd-pagination')).toHaveLength(1);
  ...
  it('should change page size work', () => {
    ...
    wrapper.find('.ezd-select-item-option').at(2).simulate('click');
