...
describe('Select', () => {
  ...
  it('should have default notFoundContent', () => {
    ...
    toggleOpen(wrapper);
    expect(wrapper.find('.ezd-select-item-option').length).toBeFalsy();
    expect(wrapper.find('.ezd-empty').length).toBeTruthy();
  });
  ...
  it('should not have notFoundContent when mode is combobox and notFoundContent is set', () => {
    ...
    toggleOpen(wrapper);
    const dropdownWrapper = mount(wrapper.find('Trigger').instance().getComponent());
    expect(dropdownWrapper.find('.ezd-select-item-option').length).toBeFalsy();
    expect(dropdownWrapper.find('.ezd-select-item-empty').at(0).text()).toBe('not at all');
  });
  ...
  it('should show search icon when showSearch and open', () => {
    ...
    expect(wrapper.find('.ezdicon-down').length).toBe(1);
    expect(wrapper.find('.ezdicon-search').length).toBe(0);
    wrapper.setProps({ open: true });
    expect(wrapper.find('.ezdicon-down').length).toBe(0);
    expect(wrapper.find('.ezdicon-search').length).toBe(1);
  });
