...
describe('click wave effect', () => {
  async function clickButton(wrapper: any) {
    wrapper.find('.ezd-btn').getDOMNode().click();
    wrapper.find('.ezd-btn').getDOMNode().dispatchEvent(new Event('transitionstart'));
    await sleep(20);
    wrapper.find('.ezd-btn').getDOMNode().dispatchEvent(new Event('animationend'));
    await sleep(20);
  }
  ...
  it('should have click wave effect for primary button', async () => {
    ...
    expect(
      wrapper.find('.ezd-btn').getDOMNode().hasAttribute('ezd-click-animating-without-extra-node'),
    ).toBe(true);
  });
  ...
  it('should handle transitionstart', async () => {
    const wrapper = mount(<Button type="primary">button</Button>);
    await clickButton(wrapper);
    const buttonNode = wrapper.find('.ezd-btn').getDOMNode();
  ...
  it('should run resetEffect in transitionstart', async () => {
    ...
    wrapper.find('.ezd-btn').getDOMNode<HTMLButtonElement>().click();
    await sleep(10);
  ...
  it('should handle transitionend', async () => {
    ...
    wrapper.find('.ezd-btn').getDOMNode().dispatchEvent(event);
    expect(resetEffect).toHaveBeenCalledTimes(2);
    resetEffect.mockRestore();
  });
