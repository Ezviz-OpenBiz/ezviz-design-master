...
describe('Progress', () => {
  ...
  it('successPercent should decide the progress status when it exists', () => {
    ...
    expect(wrapper.find('.ezd-progress-status-success')).toHaveLength(0);

    wrapper.setProps({ percent: 50, success: { percent: 100 } });
    expect(wrapper.find('.ezd-progress-status-success')).toHaveLength(1);
  ...
  it('steps should be changable', () => {
    ...
    expect(wrapper.find('.ezd-progress-steps-item-active').length).toBe(3);
    wrapper.setProps({ percent: 40 });
    expect(wrapper.find('.ezd-progress-steps-item-active').length).toBe(2);
  });

  it('steps should be changable when has strokeColor', () => {
    const wrapper = mount(<Progress steps={5} percent={60} strokeColor="#1890ff" />);
    expect(wrapper.find('.ezd-progress-steps-item').at(0).getDOMNode().style.backgroundColor).toBe(
      'rgb(24, 144, 255)',
    );
    wrapper.setProps({ percent: 40 });
    expect(wrapper.find('.ezd-progress-steps-item').at(2).getDOMNode().style.backgroundColor).toBe(
      '',
    );
    expect(wrapper.find('.ezd-progress-steps-item').at(1).getDOMNode().style.backgroundColor).toBe(
      'rgb(24, 144, 255)',
    );
  });
  ...
  it('should display correct step', () => {
    ...
    expect(wrapper.find('.ezd-progress-steps-item-active').length).toBe(4);
  });
  ...
  it('should warnning if use `progress` in success', () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      'Warning: [ezd: Progress] `success.progress` is deprecated. Please use `success.percent` instead.',
    );
  });
