...
describe('click wave effect', () => {
  async function click(wrapper) {
    wrapper.find('.ezd-switch').getDOMNode().click();
    wrapper.find('.ezd-switch').getDOMNode().dispatchEvent(new Event('transitionstart'));
    await sleep(20);
    wrapper.find('.ezd-switch').getDOMNode().dispatchEvent(new Event('animationend'));
    await sleep(20);
  }
  ...
  it('should have click wave effect', async () => {
    ...
    wrapper.find('.ezd-switch').getDOMNode().dispatchEvent(event);
