...
describe('DatePicker', () => {
  ...
  it('showTime={{ showHour: true, showMinute: true }}', () => {
    ...
    expect(wrapper.find('.ezd-picker-time-panel-column').length).toBe(2);
    expect(
      wrapper.find('.ezd-picker-time-panel-column').at(0).find('.ezd-picker-time-panel-cell')
        .length,
    ).toBe(24);
    expect(
      wrapper.find('.ezd-picker-time-panel-column').at(1).find('.ezd-picker-time-panel-cell')
        .length,
    ).toBe(60);
  });
  ...
  it('12 hours', () => {
    ...
    expect(wrapper.find('.ezd-picker-time-panel-column').length).toBe(4);
    expect(
      wrapper.find('.ezd-picker-time-panel-column').at(0).find('.ezd-picker-time-panel-cell')
        .length,
    ).toBe(12);
    ...
    expect(
      wrapper.find('.ezd-picker-time-panel-column').at(2).find('.ezd-picker-time-panel-cell')
        .length,
    ).toBe(60);
    expect(
      wrapper.find('.ezd-picker-time-panel-column').at(3).find('.ezd-picker-time-panel-cell')
        .length,
    ).toBe(2);
  });
  ...
  it('24 hours', () => {
    ...
    expect(wrapper.find('.ezd-picker-time-panel-column').length).toBe(3);
    expect(
      wrapper.find('.ezd-picker-time-panel-column').at(0).find('.ezd-picker-time-panel-cell')
        .length,
    ).toBe(24);
  ...
  it('DatePicker.RangePicker with defaultPickerValue and showTime', () => {
    ...
    const month = wrapper.find('.ezd-picker-header-view .ezd-picker-month-btn').text();
    const year = wrapper.find('.ezd-picker-header-view .ezd-picker-year-btn').text();

    expect(month).toBe(startDate.format('MMM'));
    expect(year).toBe(startDate.format('YYYY'));
    expect(wrapper.find('.ezd-picker-time-panel').length).toBe(1);
  });
});
