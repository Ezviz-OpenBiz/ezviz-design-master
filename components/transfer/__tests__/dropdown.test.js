...
describe('Transfer.Dropdown', () => {
  function clickItem(wrapper, index) {
    wrapper.find('li.ezd-dropdown-menu-item').at(index).simulate('click');
  }
  ...
  it('select all', () => {
    ...
    wrapper.find('.ezd-transfer-list-header-dropdown').first().simulate('mouseenter');
    act(() => {
      jest.runAllTimers();
    });
    wrapper.update();

    clickItem(wrapper.find('.ezd-dropdown-menu').first(), 0);
  ...
  it('select current page', () => {
    ...
    clickItem(wrapper.find('.ezd-dropdown-menu').first(), 1);
  ...
  it('should hide checkbox and dropdown icon when showSelectAll={false}', () => {
    ...
    expect(wrapper.find('.ezd-transfer-list-header-dropdown').length).toBe(0);
    expect(wrapper.find('.ezd-transfer-list-header .ezd-transfer-list-checkbox').length).toBe(0);
  });
  ...
  describe('select invert', () => {
    [
      ...
      it(name, () => {
        ...
        clickItem(wrapper.find('.ezd-dropdown-menu').first(), index);
  ...
  describe('oneWay to remove', () => {
    [
      ...
      it(name, () => {
        ...
        wrapper.find('.ezd-transfer-list-header-dropdown').last().simulate('mouseenter');
        act(() => {
          jest.runAllTimers();
        });
        wrapper.update();
