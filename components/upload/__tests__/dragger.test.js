...
describe('Upload.Dragger', () => {
  ...
  it('support drag file with over style', () => {
    ...
    wrapper.find('.ezd-upload-drag-container').simulate('dragover', {
      target: {
        files: [{ file: 'foo.png' }],
      },
    });
    ...
    expect(wrapper.find('.ezd-upload-drag').hasClass('ezd-upload-drag-hover')).toBe(true);
  ...
  it('support onDrop when files are dropped onto upload area', () => {
    ...
    wrapper.find('.ezd-upload-drag-container').simulate('drop', {
      dataTransfer: {
