...
describe('Form.List', () => {
  ...
  function testList(name, renderField) {
    it(name, async () => {
      ...
      wrapper.update();
      expect(wrapper.find('.ezd-form-item-explain div').length).toBe(1);
      ...
      operate('.remove-1');
      expect(wrapper.find(Input).length).toBe(1);
      expect(wrapper.find('.ezd-form-item-explain div').length).toBe(0);
  ...
  it('list errors', async () => {
    ...
    await addItem();
    expect(wrapper.find('.ezd-form-item-explain div').text()).toEqual('At least 2');

    await addItem();
    expect(wrapper.find('.ezd-form-item-explain div')).toHaveLength(0);
