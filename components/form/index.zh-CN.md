...
| size | 设置字段组件的尺寸（仅限 ezd 组件） | `small` \| `middle` \| `large` | - |  |
...
Form.List 本身是 renderProps，内部样式非常自由。因而默认配置 `label` 和 `error` 节点很难与之配合。如果你需要 @ezviz/ezd 样式的 `label`，可以通过外部包裹 Form.Item 来实现。
