...
The default language of `@ezviz/ezd@1.x` is currently English. If you wish to use other languages, follow the instructions below.

## ConfigProvider

`@ezviz/ezd` provides a React Component [ConfigProvider](/components/config-provider) for configuring ezd locale text globally.

```jsx
import { ConfigProvider } from '@ezviz/ezd';
import frFR from '@ezviz/ezd/lib/locale/fr_FR';
