...
Evv Design React is dedicated to providing a **good development experience** for programmers. Before starting, it is recommended to learn [React](https://reactjs.org) and [ES2015](http://babeljs.io/docs/learn-es2015/) first, and correctly install and configure [Node.js](https://nodejs.org/) v8 or above.
...
### 1. Use and modify an evvd component

Replace the contents of `index.js` with the following code. As you can see, there is no difference between evvd's components and typical React components.
...
import { DatePicker, message } from '@ezviz/evvd';
...
```diff
- import { DatePicker, message } from '@ezviz/evvd';
+ import { DatePicker, message, Alert } from '@ezviz/evvd';
```
...
OK! Now that you know the basics of using evvd components, you are welcome to explore more components in the codesandbox. When reporting a bug with evv design, we also strongly recommend using codesandbox to provide a reproducible demo as well.
...
Jest does not support `esm` modules, and Evv Design uses them. In order to test your Evv Design application with Jest you have to add the following to your Jest config :
...
`@ezviz/evvd` supports tree shaking of ES modules, so using `import { Button } from '@ezviz/evvd';` would drop js code you didn't use.
...
```
You are using a whole package of evvd, please use https://www.npmjs.com/package/babel-plugin-import to reduce app bundle size. Please upgrade webpack or check the config.
```
