...
async function checkVersion() {
  const { versions } = await fetch('http://registry.npmjs.org/@ezviz/ezd').then(res => res.json());
...
async function checkRemote() {
  ...
  if (remote.indexOf('Ezviz-OpenBiz/ezviz-design-master') === -1) {
    console.log(
      chalk.yellow('😓 Your remote origin is not Ezviz-OpenBiz/ezviz-design-master, did you fork it?'),
    );
    exitProcess();
  }
}
